...
public class EventRepository: NSObject {
...
//            onFailure(.NotFound)
        ...
        self.loading = true
        // We have this here right now until we handle the arrays of events in the repository with an observer patternr
        API.sharedInstance.getEvents(createdAtBefore: lastItemDate) { (success) in
            if success == true {
                self.loading = false
            }
        }
...
//        }
    ...
    func alreadyLoadedNewToday (tag: String, lastItemDate: String?) -> Bool {
        ...
        let activeFiltersFromModel = FilterModel.getFiltersAsDictionary(modelType: .events)
